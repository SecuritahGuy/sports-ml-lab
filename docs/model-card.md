...
| Field | Value |
|-------|-------|
| **Name** | Frozen QB Overlay (Standard Elo + qb_changed + rolling_mov_3 + Platt + frozen QB overlay) |
| **Version** | v3.0.0 |
| **Type** | Frozen-incumbent logit overlay on top of logistic regression |
...
The incumbent probability is generated by the base model (Standard Elo + qb_changed + rolling_mov_3 + Platt). A QB adjustment is applied as a logit-space overlay:

```
base_logit = logit(base_incumbent_prob)
if gate_on:
...
- **Gamma**: 1.0
- **Cap**: 40 Elo points (individual adjustments clamped to ±40)
- **Gate**: `qb_changed OR QB continuity starts < 17` (either side)
- **Non-gated games**: Exactly equal to base incumbent probability (equality check PASSED, max diff 1.11e-16)
...
| Feature | Source | Description |
|---------|--------|-------------|
| `home_qb_adj` | `compute_qb_adjustments()` | QB strength adjustment in Elo points (positive = stronger), Bayesian shrinkage toward replacement |
| `away_qb_adj` | `compute_qb_adjustments()` | QB strength adjustment in Elo points |
| `home_qb_team_starts_pre` | `compute_qb_features()` | Number of games this QB has started for this team (used for gate) |
| `away_qb_team_starts_pre` | `compute_qb_features()` | Number of games this QB has started for this team (used for gate) |
...
### Validation (fold-safe rolling average)

| Metric | Value |
|--------|-------|
| **Average log loss** | **0.6305** |
| Fold 1 (2022) | 0.6360 |
| Fold 2 (2023) | 0.6596 |
| Fold 3 (2024) | 0.5960 |
...
1. A challenger must beat **0.6200** holdout log loss to become the new football-only incumbent.
...
The closing moneyline market (no-vig) achieves holdout log loss **0.6090**, still better than the incumbent (0.6200). The market remains the true performance ceiling for pregame NFL prediction. The incumbent is a purely pregame, market-free benchmark for measuring whether independent football features can approach market efficiency.
...
| **Base incumbent** (v2.0.0, no QB overlay) | **0.6259** | **0.2177** | 0.6667 | **0.7036** |
| **Frozen QB Overlay incumbent** (v3.0.0) | **0.6200** | **0.2157** | 0.6630 | **0.7098** |

The QB overlay improves holdout log loss by **0.0059** over the base incumbent (v2.0.0),
by applying a logit-space adjustment gated to QB-change / low-continuity games only.
