...
def _run_rolling_ll(
    ...
    for train_s, val_s in ROLLING_FOLDS:
        ...
        fold_lls.append(compute_classification_metrics(
            y[va], pipe.predict_proba(x_va)[:, 1],
        )["log_loss"])
...
def run_coach_qb_tenure_experiment(
    ...
    it = results["Incumbent + tenure"]
    _w(
        f"| Incumbent + tenure | val {it['avg_val_ll']:.4f} ({'✓' if bv else '✗'})"
        f" | hold {it['holdout_ll']:.4f} ({'✓' if bh else '✗'}) | {tag} |"
    )
    _w("")

    _w("---")
    _w(
        "*Report generated by `sportslab coach-qb-tenure`."
