...
def _build_depletion_masks(
    ...
    for group in POSITION_GROUPS:
        raw_home = df.get(f"home_{group}_availability", pd.Series(1.0, index=df.index))
        raw_away = df.get(f"away_{group}_availability", pd.Series(1.0, index=df.index))
        home_avail = raw_home.values.astype(float)
        away_avail = raw_away.values.astype(float)
        hd_key = f"{group}_home_depletion"
        ad_key = f"{group}_away_depletion"
        masks[hd_key] = np.clip(1.0 - home_avail, 0, 1)
        masks[ad_key] = np.clip(1.0 - away_avail, 0, 1)
        masks[f"{group}_net_depletion"] = masks[hd_key] - masks[ad_key]
    return masks
...
def run_roster_overlay_foldsafe(
    ...
    for group in POSITION_GROUPS + ["combined"]:
        ...
        if matches:
            ...
            print(
                f"  {group}: best={best_name} ({best_val:.4f}) vs inc "
                f"({inc_val_ll:.4f}) — {beats}"
            )
    ...
    with open(rp, "w") as f:
        ...
        _w("## Methodology\n\n")
        _w(
            "For each position group, a depletion score is computed "
            "from injury report OUT counts:\n\n"
        )
        _w("```\n")
        ...
        inc_auc = inc_hold_m['roc_auc']
        inc_acc = inc_hold_m['accuracy']
        _w(
            f"| Incumbent (v3.0.0) | {inc_hold_ll:.4f} | {inc_brier:.4f} "
            f"| {inc_auc:.4f} | {inc_acc:.4f} |\n"
        )
        for name, hm in sorted(holdout_results.items(), key=lambda x: x[1]["log_loss"]):
            brier = hm['brier_score']
            auc = hm['roc_auc']
            acc = hm['accuracy']
            _w(
                f"| {name} | {hm['log_loss']:.4f} | {brier:.4f} "
                f"| {auc:.4f} | {acc:.4f} |\n"
            )
        ...
        _w(
            "4. **No per-player ratings**: Unlike the QB overlay, we don't have "
            "Bayesian-shrunken performance ratings for non-QB positions.\n\n"
        )
